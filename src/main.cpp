...
int main() {
    ...
    // --------------------
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Snoopy", NULL, NULL);
    if (window == NULL) {
    ...
    // load models
    // -----------
    Model ourModel("resources/objects/snoop/snoop.obj");
    ...
    unsigned int floorTexture = loadTexture(FileSystem::getPath("resources/textures/grassplane.jpg").c_str());
    ...
    /* ovaj kod je koriscen za inversion pre nego sto je dodat bloom
    unsigned int fbo;
    ...
    */
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        model = glm::translate(model,
                               programState->snoopyPosition); // translate it down so it's at the center of the scene
        model = glm::scale(model, glm::vec3(programState->snoopyScale));    // it's a bit too big for our scene, so scale it down
        ...
        model = glm::scale(model, glm::vec3(0.2,1.0,0.7));
        model = glm::rotate(model, glm::radians(45.0f), glm::vec3(0.0, 0.0, 1.0));
        lightCubeShader.setMat4("model", model);
        lightCubeShader.setVec3("svetlo", glm::vec3(7.0,7.0,7.0));
        ...
        glBindVertexArray(transparentVAO);
        glBindTexture(GL_TEXTURE_2D, trans_texture);
        for(glm::vec3 g : grassPos){
            model = glm::mat4(1.0f);
        ...
        shaderBloomFinal.setBool("inversion", !evil);
        glBindVertexArray(quadVAO);
        glDrawArrays(GL_TRIANGLES, 0, 6);
...
void DrawImGui(ProgramState *programState) {
...

    {
        ...
        ImGui::DragFloat("Snoopy scale", &programState->snoopyScale, 0.05, 0.1, 4.0);
